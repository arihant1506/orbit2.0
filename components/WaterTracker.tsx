  ...
  if (showConfig) {
    return (
      <div className="min-h-[70vh] flex items-center justify-center p-4 animate-fade-in-up">
        <div className="w-full max-w-2xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-[2.5rem] p-8 sm:p-12 shadow-2xl relative overflow-hidden">
           ...
           <div className="relative z-10">
             ...
             <h2 className="text-3xl sm:text-4xl font-black italic text-slate-900 dark:text-white uppercase mb-8">Adaptive <span className="text-cyan-500">Calibration</span></h2>
             ...
             <form onSubmit={handleConfigSubmit} className="space-y-8">
                ...
                <div className="p-5 rounded-2xl bg-gradient-to-br from-cyan-500/10 to-blue-500/5 border border-cyan-500/20">
                   <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                          ...
                          <h4 className="text-sm font-bold font-mono text-cyan-700 dark:text-cyan-400 uppercase tracking-wider">Medical Analysis</h4>
                      </div>
                   ...
                   <div className="flex items-end gap-4 mb-3">
                      <span className="text-5xl font-black italic text-slate-900 dark:text-white">{recommendedGoal}L</span>
                      <p className="text-xs text-slate-600 dark:text-slate-400 mb-2 max-w-[250px] leading-relaxed">
                         {medicalNote}
                      </p>
                   </div>
                </div>
                ...
                <div className="flex gap-3">
                    <button type="button" onClick={() => setGoal(recommendedGoal)} className="flex-1 py-4 bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/20 text-cyan-600 dark:text-cyan-400 rounded-xl font-bold uppercase tracking-wider transition-all text-xs">
                        Accept Recommended ({recommendedGoal}L)
                    </button>
                    <button type="submit" className="flex-1 py-4 bg-slate-900 dark:bg-white hover:bg-cyan-500 dark:hover:bg-cyan-400 text-white dark:text-slate-950 rounded-xl font-black italic uppercase tracking-wider shadow-xl transition-all flex items-center justify-center gap-2 text-xs">
      ...
      <div className="order-2 lg:order-1 h-[500px] lg:h-[600px] flex flex-col">
         ...
         <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-cyan-500/20 flex items-center justify-between">
            <div className="flex items-center gap-4">
               ...
               <div>
                  <div className="flex items-center gap-2">
                     <span className="text-2xl font-black italic text-slate-900 dark:text-white">{weather.loading ? "--" : weather.temp}°C</span>
                     <div className="h-4 w-px bg-slate-300 dark:bg-white/20" />
                     <span className="text-sm font-mono text-slate-500 dark:text-slate-400 flex items-center gap-1">
                        <Droplet className="w-3 h-3" /> {weather.loading ? "--" : weather.humidity}% RH
                     </span>
                  </div>
                  <p className="text-[9px] font-mono text-cyan-600 dark:text-cyan-400 uppercase tracking-widest mt-1">
            ...
            <div className="flex items-center gap-2">
                <button 
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/50 dark:bg-white/10 border border-slate-200 dark:border-white/10 hover:bg-white hover:shadow-sm dark:hover:bg-white/20 transition-all disabled:opacity-50"
            ...
            {slots.map((slot) => {
               ...
               return (
                  <button 
                     ...
                     disabled={isDone}
                     className={`w-full p-4 rounded-2xl border-2 transition-all duration-300 flex items-center justify-between group relative overflow-hidden ${isDone ? 'bg-cyan-500/10 border-cyan-500/50' : 'bg-white dark:bg-slate-950 border-slate-200 dark:border-white/5 hover:border-cyan-400'}`}
                  >
                     <div className="flex items-center gap-4 relative z-10">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all ${isDone ? 'bg-cyan-500 border-cyan-500 text-white' : 'border-slate-300 dark:border-slate-700 text-transparent group-hover:border-cyan-400'}`}>
                           <Check className="w-4 h-4" />
                        </div>
                        <div className="text-left">
                           <div className="flex items-center gap-2">
                              <p className={`text-lg font-black font-mono tracking-tight ${isDone ? 'text-cyan-600 dark:text-cyan-400' : 'text-slate-700 dark:text-slate-300'}`}>{slot.time}</p>
                              <div className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-widest flex items-center gap-1 ${isDone ? 'bg-cyan-500/20 text-cyan-600 dark:text-cyan-400' : 'bg-slate-100 dark:bg-white/10 text-slate-500'}`}>
